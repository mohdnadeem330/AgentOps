...
import getEvent from '@salesforce/apex/ActivityController.getMyAgencyEvents';
import   {colors} from "c/helpers";

export default class EventsUpdatesForYou extends LightningElement {
        ...
        if (data) {
          ...
          for(let i=0; i<data.length ;i++){
            ...
            this.slideData.push({
                    ...
                    bodyCardFromTime: data[i].activityDetail.StartTime__c ? this.msToTime(data[i].activityDetail.StartTime__c) : '',
            
                    bodyCardToDate: data[i].activityDetail.EndDate__c ? this.formatDate(endDate):'',
                    bodyCardToTime: data[i].activityDetail.EndTime__c ? this.msToTime(data[i].activityDetail.EndTime__c) : '',
                    subCards:
                      [{
...
*/
  ...
  openDetailsPage(event){
    document.dispatchEvent(new CustomEvent('tabsetdatafromcomponent', {detail : {showNavigationTab:false,currentStep:"events" }}));
