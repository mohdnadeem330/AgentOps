...
const mainColumns = [
    ...
    // },
    { label: 'Unit Code', fieldName: 'Name', sortable: "true" },
    ...
    //     type: 'action',
    //     typeAttributes: {
    //         rowActions: { label: 'Show details', name: 'show_details' }
    //     },
    // }
];

export default class PropertiesSearchLite extends LightningElement {
    @track columns = mainColumns;
    @track sortBy;
    @track sortDirection;
    propertyStatusPickListValues = [
        { label: "All", value: 'All' },
    ...
    async handleSearchAll(event) {
        ...
        condition += (this.propertyStatusField !== '' && this.propertyStatusField !== null && this.propertyStatusField !== 'All' && this.propertyStatusField !== undefined
            ...
            '%\''
            : (condition !== '' && condition !== null ? ' AND ' : '') + '(Status__c =  \'Available\' OR Status__c =  \'Blocked\')');
    ...
    async getDropDownData() {
        ...
        if (this.projectNameField == '' || this.projectNameField == null || this.projectNameField == undefined) {
            ...
            if (this.userRoleName != null && this.userRoleName.indexOf('Head of Sales') != -1) {
                ...
                userBlockedByQuery = 'SELECT Id,Name,ProfileName__c FROM User WHERE  (RoleName__c LIKE \'%Head of Sales%\' OR RoleName__c LIKE \'%Director of Sales%\' OR RoleName__c LIKE \'%Sales Support%\' OR id = \'' + this.loggedinUserID + '\') AND IsActive = true ';
            }
