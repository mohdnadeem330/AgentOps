...
export default class Com_ClassCreation extends NavigationMixin(LightningElement) {
    functionalAssetRTID;
    assetId;
    ...
    getAccountRecords({ error, data }) {
        if (data) {
            ...
            var newPcOptions = [];
            var allAccountsTemp = [];
            data.spAccountList.forEach(function(record){
                allAccountsTemp.push(record);
                newSpOptions.push({label:record.Name , value : record.Id});
            });

            data.rpAccountList.forEach(function(record){
                allAccountsTemp.push(record);
                newRpOptions.push({label:record.Name , value : record.Id});
            });

            data.pcAccountList.forEach(function(record){
                allAccountsTemp.push(record);
                newPcOptions.push({label:record.Name , value : record.Id});
            });

            this.spoptions = newSpOptions;
            this.rpoptions = newRpOptions;
            this.pcoptions = newPcOptions;
            this.allAccounts = allAccountsTemp;
            this.showSpRpPcBox = true;
        } else if (error) {
    ...
    showClassScreen = true;
    showClassPackageScreen = false;
    showAddMorePopup = false;
    showClassSchedule = false;
    showAddMoreSchedulesPopup = false;
    remainingCount = 32768;
    showVatAmountFields = false;

    showCommissionPercentFields = false;
    showCommissionAmountFields = false;
    ...
    handleDistrictRecordChange(event){
        console.log('inside recordpicker'+event.detail.recordId);
        this.districtId = event.detail.recordId;
    }

    handleBuildingRecordChange(event){
        console.log('inside recordpicker'+event.detail.recordId);
        this.buildingId = event.detail.recordId;
    }
    ...
    handleSubmit(event) {
        ...
        var totalCommissionPercent = parseInt(fields.Com_Aldar_ECSS_Commission__c) + parseInt(fields.Com_Related_Party_Commission__c) + parseInt(fields.Com_Service_Provider_Commission__c);
        
        if(this.serviceProviderId == undefined || this.relatedPartyId == undefined || this.profitCenterId == undefined){
            console.log('inside if');
            const classEvent = new ShowToastEvent({
                title: 'Error!',
                message: 'Please Select Service Provider, Related Party and Profit Center.',
                variant: 'error',
                mode: 'dismissable'
            });
            this.dispatchEvent(classEvent);
        }else if(this.buildingId === null || this.buildingId === undefined || this.buildingId === '' || this.districtId == null || this.districtId === undefined || this.districtId === ''){
            const classEvent1 = new ShowToastEvent({
                title: 'Error!',
                message: 'Please select Building and District for Class.',
                variant: 'error',
                mode: 'dismissable'
            });
            this.dispatchEvent(classEvent1);
        }else{
            this.showSpinner = true;
                        
            fields.Com_Service_Provider__c = this.serviceProviderId;
            fields.Com_Related_Party__c = this.relatedPartyId;
            fields.Com_Profit_Center__c = this.profitCenterId;
            fields.Com_Building__c = this.buildingId;
            fields.Com_District__c = this.districtId;

            if(this.assetId != null && this.assetId != undefined) fields.Com_Asset__c = this.assetId;

            var relatedPartyId = this.relatedPartyId;
            var profitCenterId = this.profitCenterId;

            this.allAccounts.forEach(function(record){
                if(record.Id == relatedPartyId){
                    fields.Com_Related_Party_Code__c = record.SapAccountID__c;
                }
                if(record.Id == profitCenterId){
                    fields.Com_Vendor_Code__c = record.SapAccountID__c;
                }
            })
    ...
    handldePicklistchange(event){

        if(event.detail.selectedlabel == 'Service Provider'){
            this.serviceProviderId = event.detail.selectedValue;
        }else if(event.detail.selectedlabel == 'Related Party'){
            this.relatedPartyId = event.detail.selectedValue;
        }else if(event.detail.selectedlabel == 'Profit Center'){
            this.profitCenterId = event.detail.selectedValue;
        }
    }

    checkValues(event){
        console.log('inside checkvalues');
        console.log(this.serviceProviderId);
        console.log(this.relatedPartyId);
        console.log(this.profitCenterId);
    }

    handleSuccess(event) {
        console.log('onsuccess event recordEditForm',event.detail.id);
        this.classId = event.detail.id;
    ...
    handleUploadFinished(event) {
        ...
        createDocuments({asseId : this.classId, documentIdList : documentIds})
        .then(data => {
    ...
    handleClassPackageSubmit(event) {
        ...
        if(requiredFieldsMissing){
            ...
            this.showSpinner = true;
            createClassPackages({classId : this.classId, packageList : this.itemList})
            .then(result => {
                var newOptions = [];
                setTimeout(() => {
    ...
    handleClassScheduleSuccess(event) {
        ...
        updateSchedule({classId : this.classId, packgeList : this.classPackagesList, scheduleNamesList: this.scheduleNamesList, scheduleId : event.detail.id})
        .then(result => {
            const classScheduleEvent = new ShowToastEvent({
            title: 'Success!',
            message: 'Class schedule has been succesfully added!.',
            variant: 'success',
            mode: 'dismissable'
            });
            this.dispatchEvent(classScheduleEvent);
    ...
    /*handleScheduleOkay(event){
        this.showAddMoreSchedulesPopup = false;
        this.showClassSchedule = true;
    }

    handleScheduleNotOkay(event){
        this.showAddMoreSchedulesPopup = false;
    ...
    handlePackageFeesChange(event){
        this.packageFeesList[event.target.accessKey] = event.detail.value;
        console.log(this.packageFeesList);
    }
    ...
    handleCommunityChange(event){
        var communityId = event.target.value;
        ...
        this.showBuildingDistrict = true;
    }
