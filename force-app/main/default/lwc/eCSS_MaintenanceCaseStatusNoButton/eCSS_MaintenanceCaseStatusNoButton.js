...
import STATUS_FIELD from '@salesforce/schema/Case.Sub_Status__c';
...
export default class ECSS_MaintenanceCaseStatusNoButton extends LightningElement {
    ...
    registerEmpListener() {
        const messageCallback = (response) => {
            if(response)
            {
                ...
                else if(CaseStatusValue.length>0 && CaseStatusValue[0]=='Substatus')
                {
   ...
   // Wire adapter to fetch picklist values for STATUS_FIELD field
    @wire(getPicklistValues, { recordTypeId: '$RecordTypeIdval', fieldApiName: STATUS_FIELD })
    wiredPicklistValuesStatus(value) {

            this.picklistStatusFieldValues = value
            console.log('wiredPicklistValuesStatus--StatusField:', JSON.stringify(this.picklistStatusFieldValues));
    }

    // Wire adapter to fetch picklist values for RESOLVED
    @wire(getPicklistValues, { recordTypeId: '$RecordTypeIdval', fieldApiName: RESOLVED })
    wiredPicklistValuesResolved({ data, error }) {
        if (data) {
            
            console.log('wiredPicklistValuesResolved-Resolved:', JSON.stringify(data));
            this.picklistResolvedValues = data.values.map(item => ({
                label: item.label,
                value: item.value
            }));
            console.log('wiredPicklistValuesResolved-Resolved--Picklist:', JSON.stringify(this.picklistResolvedValues));
        } else if (error) {
    ...
    @wire(getPicklistValues, { recordTypeId: '$RecordTypeIdval', fieldApiName: SUB_STATUS_FIELD })
    wiredPicklistValuesSubstatus(result) {
        
        this.wiredSubstatusResult = result;  // Store the result
        this.tempVariable = result;
        console.log('wiredPicklistValuesSubstatus-SUB_STATUS_FIELD:', JSON.stringify(this.picklistResolvedValues));
        if(result.data)
        {
            ...
            console.error('wiredPicklistValuesSubstatus-SUB_STATUS_FIELD-error:', JSON.stringify(result.error));
        }
         ...
         if(this.record.data.fields.Status.value == 'Resolved')
             {
                console.log('--Resolved---');
                 ...
                 console.log('--Resolved -PickList ---'+JSON.stringify(this.picklistSubstatusValues));
             }
             else if(this.record.data.fields.Status.value == 'New' || this.record.data.fields.Status.value == 'Closed')
             {
                console.log('--New -Closed ---');
                     ...
                     console.log('--New -Closed ---'+JSON.stringify(this.picklistSubstatusValues));
             }

             else{
                console.log('--Last  ---');
                 this.picklistSubstatusValues = substatusval.values.filter(option => {
                     return option.validFor.includes(1) || option.validFor.includes(2) || option.validFor.includes(3);
                     })
                     console.log('--Last  ---'+JSON.stringify(this.picklistSubstatusValues));
             }
             return null;
             console.log('Test pIackslistsubstatus***' +JSON.stringify(this.picklistSubstatusValues));
    ...
    handleUpdateRecord(event){
       ...
       console.log('---value from Child recordInput-- '+  JSON.stringify(recordInput));
        updateRecord(recordInput)
            .then((result) => {
                ...
                console.log('--parent picklistSubstatusValues--' + JSON.stringify(this.picklistSubstatusValues));
                console.log('success!');
    ...
    handleError(error) {
        ...
        this.showToast('Error', 'Failed to update record: ' + this.errorMessages.join(', '), 'error');
    }
